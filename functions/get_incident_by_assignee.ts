...
export default async ({ token, inputs, env, }: any) => {
  try {
    ...
    if (inputs.assigned_to) {
      console.log('inside assigned_to')
      url = url.concat("=assigned_to.link%3Dhttps://dev88853.service-now.com/api/now/table/sys_user/" + inputs.assigned_to)
    }
    if (inputs.limit) {
    ...
    //https://dev88853.service-now.com/api/now/table/incident?sysparm_query=assigned_to.value%3DU03GLTFNLQH&sysparm_limit=10
//active=true^GOTOassigned_to.name>=U03GLTFNLQH
    //mine U0368LRBZ44
